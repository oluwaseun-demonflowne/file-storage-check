...
export default async function Page() {
  ...
  await queryClient.prefetchQuery({
    ...
    queryFn: () =>
      axios
        .get(`/api/CompanyFiles/${user?.emailAddresses[0]?.emailAddress}`)
        .then((res) => res.data),
  });
  const dehydratedState = dehydrate(queryClient);
  // const {companyName , error} = await getPublicFiles(user?.emailAddresses[0]?.emailAddress)
  // if(error) {
  //     user && await getPublicFiles(user?.emailAddresses[0]?.emailAddress)
  // }
  return (
    <section className="px-4 md:px-10 mb-10 flex flex-col gap-7">
      <div className="w-100%">
